...
export default function TimeCard({ date="22/06", time="12:00", temperature=0, code, className="" }) {
  ...
  backdrop-blur-xl bg-white/10 border border-white/20 shadow-2xl hover:scale-105 hover:shadow-lg hover:bg-white/20 ${className}`}>

            <h3 className="text-lg text-center font-sans">{date}</h3>
            <h3 className="text-sm text-center font-sans">{time}</h3>

            <div className="gap-4 relative">
                <img src={`/icons/${getIconForWeatherCode(parseInt(code), true)}`} alt="" className="w-20 scale-[105%]" />
